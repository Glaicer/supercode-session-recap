...
Чистая логика модели (`parseModelRef`, опции, валидация) вынесена в `recap-model.ts`
без TUI-импортов и покрыта юнит-тестами: `node --test recap-model.test.ts`.
...
| `budget` | number | `12000` | бюджет Digest в символах (потребитель — тикет 03) |
...
```bash
node --test recap-model.test.ts        # юнит-тесты чистой логики (без TUI)
